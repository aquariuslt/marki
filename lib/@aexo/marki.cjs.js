...
var MarkiConfig = function MarkiConfig() {
  _classCallCheck(this, MarkiConfig);
};
...
fancyLog.info = info_1;
fancyLog.dir = dir_1;
fancyLog.warn = warn_1;
fancyLog.error = error_1;
...
var metadataRule = {
   ...
   * */
  lex: function lex(context) {
    var $this = this;
    var tokens = context.tokens;
    var metadataToken = lodash.find(tokens, META_TOKENS);

    if (lodash.isUndefined(metadataToken)) {
      fancyLog.error('Could not found metadata');
      return;
    }
    context['metadata'] = JSON.parse(metadataToken.text);

    if (!lodash.isUndefined($this.ignore) && $this.ignore) {
      lodash.remove(tokens, META_TOKENS);
    }
  }
};
...
function load(markiConfig) {
  updateContext({ config: markiConfig });
}
